  ...
  while(1)
  {
    ...
    case 2: printf("Enter the word for deletion..\n");
       ...
       case 6:length=0;
              ...
              break;
		case 7: destroy_trie(root);
      }
  }
 
 }

  void destroy_trie(struct trienode *curr)
  {
	  int i;
	  for(i=0;i<256;i++)
	  {
		  if(curr->child[i]!=NULL)
			  destroy_trie(curr->child[i]);
	  }
	  free(curr);
  }
...
void display_prefix(struct trienode *root, char *prefix)
{
	...
	for(i=0;prefix[i]!='\0';i++)
	{
	  ...
	  {
         printf("No word having prefix %s in the trie",prefix);
        return;
	  }
	}
    if(curr->endofword==1)
...
void display_length(struct trienode *curr, int count)
{
	...
	for(i=0;i<256;i++)
	{
		...
		{
			word[length++]=i;
			if((curr->child[i]->endofword==1)&&(length==count))
			{
				printf("\n");
				for(j=0;j<length;j++)
					printf("%c",word[j]);
